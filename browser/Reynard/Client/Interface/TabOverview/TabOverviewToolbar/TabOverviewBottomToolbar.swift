...
final class TabOverviewBottomToolbar: UIView {
    ...
    private let tabModeControl = UISegmentedControl(items: [L("Private"), String(format: L("%d Tabs"), 0)])
    ...
    func apply(tabCount: Int, hasVisibleTab: Bool) {
        tabModeControl.setTitle(String(format: L("%d Tabs"), tabCount), forSegmentAt: TabOverview.Mode.regularTabs.rawValue)

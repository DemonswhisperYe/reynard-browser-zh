...
final class DownloadsCoordinator {
    ...
    private func presentNextConfirmationAlertIfNeeded() {
        ...
        AlertPresenter.show(
            title: String(format: L("Do you want to download \"%@\"?"), pendingDownload.fileName),
            message: nil,
            buttons: [
                AlertPresenter.Button(title: L("Cancel"), style: .cancel) { [weak self] in
                    self?.resolveConfirmation(shouldStartDownload: false)
                },
                AlertPresenter.Button(title: L("Download")) { [weak self] in
                    Haptics.success()

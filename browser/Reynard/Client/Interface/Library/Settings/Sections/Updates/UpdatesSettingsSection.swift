...
final class UpdatesSettingsSection {
    ...
    func cell(at index: Int, allowUpdate: Bool, tintColor: UIColor?) -> UITableViewCell {
        ...
        case .updateNow:
            let cell = SettingsViewUtils.actionCell(title: L("Update Now"), tintColor: tintColor)
    ...
    func trollStoreFooterView() -> UIView {
        return footerView(text: L("Make sure TrollStore's URL Scheme is enabled."))
    }
    ...
    private func beginUpdate(from viewController: UIViewController) {
        guard let updateFeedData = BrowserUpdates.shared.sourceData,
            ...
            AlertPresenter.show(title: L("Update Unavailable"), message: L("Could not retrieve the download URL."))
            return
        }
        ...
        if installedThroughTrollStore {
            ...
            downloadUpdate(
                ...
                message: L("When the download finishes, choose the app that you used to sideload Reynard in the share sheet to install the update."),
    ...
    private func downloadUpdate(
        ...
        let alert = UIAlertController(title: L("Downloading Update"), message: message, preferredStyle: .alert)

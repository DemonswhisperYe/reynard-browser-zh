        ...
        return UIContextMenuConfiguration(identifier: bookmark.guid as NSString, previewProvider: nil) { _ in
            UIMenu(title: "", children: [
                UIMenu(title: "", options: .displayInline, children: [
                    UIAction(title: L("Open in New Tab"), image: UIImage(named: "reynard.plus.square.on.square")) { _ in
                        openInNewTab()
                    },
                    UIAction(title: L("Open in New Private Tab"), image: UIImage(named: "reynard.plus.square.on.square")) { _ in
                        openInNewPrivateTab()
                    },
                ]),
                UIMenu(title: "", options: .displayInline, children: [
                    UIAction(title: L("Copy Link"), image: UIImage(named: "reynard.document.on.document")) { _ in
                        UIPasteboard.general.string = bookmark.url.absoluteString
                    },
                    UIAction(title: L("Share Link"), image: UIImage(named: "reynard.square.and.arrow.up")) { _ in
                        shareLink(bookmark.url)
                    },
                ]),
                UIMenu(title: "", options: .displayInline, children: [
                    UIAction(title: L("Edit Bookmark"), image: UIImage(named: "reynard.pencil")) { _ in
                        editBookmark()
                    },
                    UIAction(
                        title: L("Delete Bookmark"),
